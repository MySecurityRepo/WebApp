...
const ad = ref(null)
const slot = ref('sports_tablet_320x400')
const ad_type = "sports"
...
useHead({
  ...
  link: [
    {
      rel: "canonical",
      href: "https://thebooksclub.com/sports"
    }
  ]
})
...
const pickResponsiveSlot = () => {
  ...
  if (w <= 450) slot.value = 'sports_mobile_224x150'
  else if (w <= 1024) slot.value = 'sports_tablet_320x400'
  else slot.value = 'sports_desktop_500x630'
}
...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-sport-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('sportvue.title') }} ⚽</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('sportvue.header') }}</p>
      ...
      <div class="card card-width mx-auto border border-2 border-warning rounded-2 mb-5" >
        <div class="card-body"> 
          ...
          <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center">
            ...
            <RouterLink :to="`/user/${item.author_id}`" class="order-1 order-sm-2 text-decoration-none">
              <p class="text-danger order-1 order-sm-2 text-start" style="font-size: 0.9rem;">{{ item?.is_modified ? t('homevue.modified_by') : t('homevue.created_by') }} {{ item?.author_username }} {{ t('homevue.at') }} {{ item?.created }}</p>
            </RouterLink>
          </div>
