...
const ad = ref(null)
const slot = ref('tech_tablet_320x400')
const ad_type = "tech"
...
useHead({
  ...
  link: [
    {
      rel: "canonical",
      href: "https://thebooksclub.com/tech-science"
    }
  ]
})
...
const pickResponsiveSlot = () => {
  ...
  if (w <= 450) slot.value = 'tech_mobile_224x150'
  else if (w <= 1024) slot.value = 'tech_tablet_320x400'
  else slot.value = 'tech_desktop_500x630'
}
...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-tech-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('techvue.title') }} 🚀</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('techvue.header') }}</p>
