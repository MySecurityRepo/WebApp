...
const ad = ref(null)
const slot = ref('books_tablet_320x400')
const ad_type = "books"
...
useHead({
  ...
  link: [
    {
      rel: "canonical",
      href: "https://thebooksclub.com/books"
    }
  ]
})
...
const pickResponsiveSlot = () => {
  ...
  if (w <= 450) slot.value = 'books_mobile_224x150'
  else if (w <= 1024) slot.value = 'books_tablet_320x400'
  else slot.value = 'books_desktop_500x630'
}
...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-book-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('bookvue.title') }} 📖</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('bookvue.header') }}</p>
