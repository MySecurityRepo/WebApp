...
const ad = ref(null)
const slot = ref('music_tablet_320x400')
const ad_type = "music"
...
useHead({
  ...
  link: [
    {
      rel: "canonical",
      href: "https://thebooksclub.com/art-music"
    }
  ]
})
...
const pickResponsiveSlot = () => {
  ...
  if (w <= 450) slot.value = 'music_mobile_224x150'
  else if (w <= 1024) slot.value = 'music_tablet_320x400'
  else slot.value = 'music_desktop_500x630'
}
...
function enhanceMediaHTML(rawHTML, attachments = []) {
  ...
  doc.querySelectorAll('attachment').forEach(node => {
    ...
    if ((kind === 'video') || (att.mime && att.mime.startsWith('video/'))) {
      ...
      v.setAttribute('poster', att.thumnail);
      node.replaceWith(v);
      return;
    }
...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-art-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('artvue.title') }} 🎭</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('artvue.header') }}</p>
