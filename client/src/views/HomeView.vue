...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">Home 🏠</h1>
    ...
    <p class="text-center mt-4 p-outside-large">{{ t('homevue.header') }}</p>
        ...
        <div class="card-body"> 
          ...
          <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center">
            <div class="margin-post-buttons order-2 order-sm-1">
              ...
              <button class="btn-like-post border-0 bg-transparent btn-like-post" @click="commentsByPost[item.post_id]?.items.length ? resetComments(item.post_id) : showParentComments(item)" :disabled="commentsByPost[item.post_id]?.loading">
                <i :class="commentsByPost[item.post_id]?.items.length ? 'bi-dash-circle text-danger' : 'bi-plus-circle text-danger'" class="bi i-post-buttons"></i>
              </button>
