...
const ad = ref(null)
const slot = ref('events_tablet_320x400')
const ad_type = "events"
...
useHead({
  ...
  link: [
    {
      rel: "canonical",
      href: "https://thebooksclub.com/social"
    }
  ]
})
...
const pickResponsiveSlot = () => {
  ...
  if (w <= 450) slot.value = 'events_mobile_224x150'
  else if (w <= 1024) slot.value = 'events_tablet_320x400'
  else slot.value = 'events_desktop_500x630'
}
...
async function setupObserver() {
  await nextTick() // ensure #sentinel exists
...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-social-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('socialvue.title') }} 🎉</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('socialvue.header') }}</p>
