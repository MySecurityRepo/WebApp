...
async function loadMore() {
  ...
  try{
    const res = await api.get(`/retrieve-favorite-posts/${page.value}`)
...
<template>
  <div class="container-fluid mt-3 mx-0 px-0">
    <h1 class="text-warning fw-bold">{{ t('favoritesvue.title') }} 💕</h1>
    ...
    <p class="text-center mt-4 p-outside-large"> {{ t('favoritesvue.header') }}</p>
          ...
          <div v-if="item.isreplying && (replyingId === item.post_id)" class="mb-4">
            <div class="w-50 comment-toolbar"> 
              <div class="comment-toolbar bg-dark border border-2 rounded-top xs-limited-width-alert position-relative d-flex align-items-stretch">
                ...
                <button type="button" class="btn-like border btn-attch bg-dark text-toolbar1" @click="selectFile(editor)" :class="{'btn-active': isFileActive}" :disabled="!editor || editorLoading">
